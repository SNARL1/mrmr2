functions {
  ...
  matrix forward_prob(int i, // individual index
                      ...
                      vector logit_detect, vector eps_detect_indiv,
                      vector lambda, vector gam_init,
    ...
    for (j in 1 : Jtot) {
      p = inv_logit(logit_detect[j] + eps_detect_indiv[i]);
      po[1, j, 1] = 1;
      po[1, j, 2] = 0;
  ...
  real partial_sum_lpmf(array[] int slice_individuals, int start, int end,
    ...
    for (i in start : end) {
      forward = forward_prob(i, X_surv, beta_phi, eps_phi, logit_detect,
                             eps_detect_indiv, lambda, gam_init, introduced,
                             t_intro, removed, t_remove, prim_idx,
                             any_surveys, J, j_idx, Y, Jtot, T);
...
parameters {
  ...
  real<lower=0> sigma_detect_indiv;
  vector<multiplier=sigma_detect_indiv>[M] eps_detect_indiv; // individual heterogeneity (Mh)
}
...
model {
  ...
  sigma_detect_indiv ~ std_normal();
  eps_detect_indiv ~ normal(0, sigma_detect_indiv);
  beta_phi ~ std_normal();
  sigma_phi ~ std_normal();
  eps_phi ~ normal(0, sigma_phi);

  target += reduce_sum(partial_sum_lupmf, Mseq, grainsize, X_surv, beta_phi,
                       eps_phi, logit_detect, eps_detect_indiv, lambda,
                       gam_init, introduced, t_intro, removed, t_remove,
                       prim_idx, any_surveys, J, j_idx, Y, Jtot, T);
}
  ...
  
  {
    ...
    for (i in 1 : M) {
      ...
      forward_probabilities = forward_prob(i, X_surv, beta_phi, eps_phi,
                                           logit_detect, eps_detect_indiv,
                                           lambda, gam_init, introduced,
                                           t_intro, removed, t_remove,
                                           prim_idx, any_surveys, J, j_idx,
                                           Y, Jtot, T);
